...
fn update_ticket(client: &Client, token: &str){
    let id = input("ENTER THE TICKET ID : ");
    let title = input("ENTER NEW TITLE(leave blank to skip) : ");
    let description = input("ENTER NEW DESCRIPTION(leave blank to skip) : ");
    ...
    let res = client.put(format!("{}/tickets/{}",BASE_URL,id))
    .bearer_auth(token)
    .json(&update)
    .send();

    match res {
        Ok(r) if r.status().is_success() => println!("Ticket Updated Successfully !!"),
        Ok(r) => println!("❌ Error: {}", r.text().unwrap_or_default()),
        Err(e) => println!("❌ Network error: {}", e),
    }
}
